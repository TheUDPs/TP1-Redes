...
Durante la etapa de transferencia del archivo, la parte de la transmisión que actúa de sender tiene la responsabilidad de re-enviar toda la ventana que tiene disponible desde el siguiente paquete al último que haya sido confirmado con un ACK (conocido como base) por parte del receiver. Para que el sender pueda considerar un paquete correctamente confirmado debe tener el mismo ACK number que está esperando recibir o que haya llegado un ACK con un número mayor, por lo que se entiende que el receiver recibió correctamente todos los paquetes hasta el ACK number que envío. Esto último despeja de repente una buena porción de la ventana para enviar consecutivamente nuevos paquetes. El tamaño de la ventana utilizado, que mediante comprobación empírica es el que mejor resultado trajo, es de tamaño de 10 paquetes.
...
Se observa un aumento de tiempo en la transmisión a medida que aumenta el porcentaje de pérdida de paquetes, debido a que la aplicación debe tardar cada vez más tiempo en realizar las retransmisiones debido a un mayor volumen de paquetes perdidos.
...
Se observa el mismo aumento de tiempo explicado para la operación de upload. También se detalla una gran similaridad de tiempos entre las dos operaciones, ya que en esencia, se está realizando lo mismo pero lados distintos de la arquitectura cliente-servidor.
...
Capturando el output del comando iperf (ejecutado durante 7 segundos) se puede observar el resultado final de la comunicación en cliente y servidor:
...
La performance se ve significativamente comprometida comparado a un escenario sin fragmentación ni pérdida de paquetes, situación donde el bitrate alcanza sin mayores problemas los 28 Gbits/s enviando 23 GB en los 7 segundos. Este comportamiento es parte del mismo control de flujo y congestión de TCP.
