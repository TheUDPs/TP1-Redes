...
def run_automated_test(protocol, h1, h2, r2):
    ...
    pid1 = run_tcpdump(r2, 'r2-eth0', f'{CAPTURE_DIR}/router_eth0_{protocol_lower}.pcap')
    pid2 = run_tcpdump(r2, 'r2-eth1', f'{CAPTURE_DIR}/router_eth1_{protocol_lower}.pcap')
    ...
    if protocol == 'TCP':
        client_output = h2.cmd(f'iperf -c 10.0.0.1 -t {TEST_DURATION} -i 1 -M {FRAG_MSS}')
    else:  
        client_output = h2.cmd(f'iperf -c 10.0.0.1 -u -b 10m -l {FRAG_MSS} -t {TEST_DURATION} -i 1')
    
    with open(f'{CAPTURE_DIR}/iperf_{protocol_lower}_client.log', 'w') as f:
        f.write(client_output)
    
    info(f'Stopping tcpdump and iperf processes...\n')
